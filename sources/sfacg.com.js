...
const headers = ["authorization:Basic YW5kcm9pZHVzZXI6MWEjJDUxLXl0Njk7KkFjdkBxeHE=","accept:application/vnd.sfacg.api+json;version=1",`user-agent:boluobao/4.9.24(android;31)/XIAOMI/${devicetoken}/XIAOMI`,`sfsecurity:nonce=${nonce}&timestamp=${timestamp}&devicetoken=${devicetoken.toUpperCase()}&sign=${sign}`]

//搜索
const search = (key) => {
  let response = GET(`https://api.sfacg.com/search/novels/result/new?q=${key}&expand=novels,comics,albums,chatnovelstags,typeName,authorName,intro,latestchaptitle,latestchapintro,tags,sysTags&sort=hot&page=0&size=12&systagids=`,{headers})
...
const detail = (url) => {
  ...
  let book = {
    ...
    category: $.expand.sysTags.map((item)=>{ return item.tagName}).join(" ") || $.expand.typeName,
    words: $.charCount,
...
//目录
const catalog = (url) => {
  let response = GET(`https://api.sfacg.com/novels/${url}/dirs?expand=originNeedFireMoney`,{headers})
  let $ = JSON.parse(response)
  let array = []
...
//章节
const chapter = (url) => {
  let response = GET(`https://api.sfacg.com/Chaps/${url}?expand=content,needFireMoney,originNeedFireMoney,tsukkomi,chatlines,isbranch&autoOrder=true`,{headers})
  let $ = JSON.parse(response)
  //未购买返回403和自动订阅地址
 ...
 */
const profile = () => {
  let response = GET(`https://api.sfacg.com/user?expand=vipInfo,welfareCoin,isRealNameAuth,realnameinfo,changeNickNameInfo,welfareMoney,redpacketCode,useWelfaresys,usedRedpacketCode,hasOrderChapWithFireMoney,hasUnlockChapWithAd,hasActiveUnlockChapWithAd,hasOrderedVipChaps,hasPaidFirstTime,growup,newVip`,{headers})
  let response2 = GET(`https://api.sfacg.com/user/money`,{headers})
  let $ = JSON.parse(response).data
...
const bookshelf = (page) => {
  let response = GET(`https://api.sfacg.com/user/Pockets?expand=novels,albums,comics,discount,discountExpireDate`,{headers})
...
const rank = (title, category, page) => {
  let response = GET(`https://api.sfacg.com/novels/${title}/sysTags/novels?expand=typeName,sysTags,intro,discount,discountExpireDate&updatedays=-1&size=20&isfree=both&charcountbegin=0&systagids=&sort=latest&page=${page}&isfinish=both&charcountend=0`,{headers})
...
const login = (args) => {
  ...
  let response = POST(`https://api.sfacg.com/sessions`,{data,headers})
  let $ = JSON.parse(response)
