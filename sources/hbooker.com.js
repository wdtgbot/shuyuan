 ...
 */
const catalog = (url) => {
    let result = CGET('chapter/get_updated_chapter_by_division_new', {
        division_id: 0,
        book_id: url
    }).chapter_list
    let array = []
    result.forEach((volume) => {
        array.push({
            name: volume.division_name,
        })
        let $ = volume.chapter_list.filter(function(item) {
...
const login = (args) => {
    ...
    let response = POST("https://sk.hbooker.com/signup/login", {
        data: `login_name=${args[0]}&app_version=${app_version}&passwd=${args[1]}&device_token=${device_token}`,
        headers
    })
    let $ = JSON.parse(decrypt(response))
    if ($.code == "100000") {
