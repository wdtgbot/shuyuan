...
const headers = ['platform: android','app-version: 20400','application-id: com.xm.freader',`sign: ${CryptoJS.MD5("app-version=20400application-id=com.xm.freaderplatform=androidd3dGiJc651gSQ8w1")}`,'user-agent:webviewversion/0']
...
const search = (key) => {
  ...
  let response = GET(`https://api-bc.kureader.com/api/v7/search/words?wd=${encodeURI(key)}&sign=${sign}`,{headers})
  let array = []
  let $ = JSON.parse(response)
  $.data.books.filter($ => $.show_type != 4 && $.show_type != 2 && $.show_type != 3).forEach(($) => {
    array.push({
      name: $.original_title,
...
const detail = (url) => {
  ...
  let response = GET(`https://api-bc.kureader.com/api/v5/book/detail?id=${url}&sign=${sign}`,{headers})
...
const catalog = (url) => {
  let sign = CryptoJS.MD5(`id=${url}d3dGiJc651gSQ8w1`)
  let response = GET(`https://api-ks.kureader.com/api/v1/chapter/chapter-list?id=${url}&sign=${sign}`,{headers})
  let $ = JSON.parse(response)
  let array = []
...
const chapter = (url) => {
  ...
  let response = GET(`https://api-ks.kureader.com/api/v1/chapter/content?${url}&sign=${sign}`,{headers})
  let $ = JSON.parse(response)
