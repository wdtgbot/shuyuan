...
const search = (key) => {
  ...
  let response = GET(`https://api-bc.wtzw.com/api/v7/search/words?wd=${encodeURI(key)}&sign=${sign}`,{headers})
  let array = []
...
const detail = (url) => {
  ...
  let response = GET(`https://api-bc.wtzw.com/api/v5/book/detail?id=${url}&sign=${sign}`,{headers})
...
const chapter = (url) => {
  ...
  let response = GET(`https://api-ks.wtzw.com/api/v1/chapter/content?${url}&sign=${sign}`,{headers})
  let $ = JSON.parse(response)
